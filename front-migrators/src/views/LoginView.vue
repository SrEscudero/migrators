...
import { useRouter } from 'vue-router'; // AJUSTE: Importamos useRouter aquí.

const authStore = useAuthStore();
const router = useRouter(); // AJUSTE: Obtenemos la instancia del router aquí.
const email = ref('');
const password = ref('');
...
const handleLogin = async () => {
  ...
  try {
    // AJUSTE: La acción de login ahora devuelve los datos del usuario.
    const loggedInUser = await authStore.login({
      email: email.value,
      password: password.value,
    });

    // AJUSTE: El componente decide a dónde redirigir.
    if (loggedInUser) {
      if (loggedInUser.rol === 'ceo' || loggedInUser.rol === 'funcionario') {
        router.push('/admin');
      } else {
        router.push('/');
      }
    ...
    errorMessage.value = error.response?.data?.message || 'Credenciales inválidas o error de red.';
  } finally {
